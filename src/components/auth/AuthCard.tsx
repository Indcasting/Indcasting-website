    ...
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative' }}>
      ...
      <div style={{ width: '100%', maxWidth: '480px', position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', textDecoration: 'none', color: 'var(--text)', marginBottom: '24px' }}>
            <Camera size={28} color="var(--gold)" />
            <span style={{ fontSize: '1.5rem', fontWeight: 800, letterSpacing: '-0.5px' }}>IndCasting</span>
          </Link>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text)', marginBottom: '8px' }}>{title}</h1>
          <p style={{ color: 'var(--dash-text-muted)', fontSize: '1.05rem' }}>{subtitle}</p>
        </div>

        {children}

        <p style={{ textAlign: 'center', marginTop: '32px', color: 'var(--dash-text-muted)', fontSize: '0.95rem' }}>
          {bottomText}{' '}
