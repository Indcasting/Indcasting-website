...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  // Accept scrollTop directly from Lenis event — no re-read of window.scrollY
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      // Use scroll value directly from Lenis — avoids window.scrollY mismatch
      ...
      // Expose lenis so the parent (page.tsx) can drive it from GSAP ticker
      onLenisReady?.(lenis);
      ...
      // For contained scroll mode, drive Lenis ourselves
      let rafId: number;
  ...
  }, [updateCardTransforms, useWindowScroll, onLenisReady]);
  ...
  useLayoutEffect(() => {
    ...
    cards.forEach((card, i) => {
      ...
      // Start with a proper base transform — no stale translateZ(0) fighting later writes
      card.style.transform = 'translate3d(0, 0, 0)';
    });

    const cleanupLenis = setupLenis();

    // Initial render pass using current scroll position
    const initialScroll = useWindowScroll ? window.scrollY : (scroller.scrollTop ?? 0);
